...
// カメラ用の変数
Capture cam;
int w = 640, h = 480, hist_w = 300, hist_h = 180, shiftPix = 3;

PImage srcImg = null;
PGraphics histImg;
float histScale; // ヒストグラムのグラフを表示する際の倍率

void settings(){
  // 画像の配置を考慮したウィンドウサイズ
  size(w + hist_w + shiftPix, hist_h * 3 + shiftPix * 2);
}
...
void setup(){
  ...
  histImg = createGraphics(hist_w, hist_h);
  histScale = hist_w / 256.0;
}
...
void draw(){
  ...
  image(histImg, w + shiftPix, hist_h + shiftPix);
  
  // 青ヒストグラムを算出してグラフを画像化
  genHistogram(srcImg, histImg, histScale, 2);
  image(histImg, w + shiftPix, hist_h * 2 + shiftPix * 2);
}
